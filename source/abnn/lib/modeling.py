...
resources = Resources(number_node=1, cpu_per_node=8, gpu_per_node=1, queue_name="GPU_2080Ti", group_size=1, if_cuda_multi_devices=False) 
res_resources = Resources(number_node=1, cpu_per_node=8, gpu_per_node=1, queue_name="GPU_2080Ti", group_size=10, if_cuda_multi_devices=False) 
cpu_resources = Resources(number_node=1, cpu_per_node=1, gpu_per_node=0, queue_name="GPU_2080Ti", group_size=1, if_cuda_multi_devices=False) 
...
##############################################################################################
        ...
        itp_files = glob.glob(templ_mol_path + "*.itp")
...
def run_res (iter_index,
    ...
    gmx_prep_submission = Submission(work_base=res_path, resources=res_resources, machine=slurm, task_list=gmx_prep_task)
    ...
    gmx_run_submission = Submission(work_base=res_path, resources=res_resources, machine=slurm, task_list=gmx_run_task)
...
def post_res (iter_index,
    ...
    cmpf_resources = Resources(number_node=1, cpu_per_node=1, gpu_per_node=0, queue_name="GPU_2080Ti", group_size=group_size, if_cuda_multi_devices=False)
    lazy_local_context = LazyLocalContext(local_root='./')
    slurm = Slurm(context=lazy_local_context)
    cmpf_task = [ Task(command=cmpf_cmd, task_work_path="./{}".format(ii), outlog='cmpf.out', errlog='cmpf.err') for ii in all_task ]
    cmpf_submission = Submission(work_base='./', resources=cmpf_resources, machine=slurm, task_list=cmpf_task)
...
def run_train (iter_index, 
    ...
    train_cmd = "python ../main.py -t %d" % train_thread
    train_cmd += cmdl_args
    train_cmd = cmd_append_log (train_cmd, "train.log")
    freez_cmd = "python ../freeze.py -o graph.pb"
    ...
    train_submission = Submission(work_base=train_path, resources=resources, machine=slurm, task_list=train_task)
    ...
    freez_submission = Submission(work_base=train_path, resources=resources, machine=slurm, task_list=freez_task)
