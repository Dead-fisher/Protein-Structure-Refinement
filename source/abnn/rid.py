...
from lib.score import score_process
...
def add_distance_restrain(residue_atoms, distance_list, dis_buttom = 0.2, dis_kappa = 10.467):
    ...
    for dis in distance_list:
        ret += "dis-{:d}-{:d}: DISTANCE ATOMS={},{}".format(dis[0], dis[1], residue_atoms[dis[0]], residue_atoms[dis[1]])
        ret += "\n"
...
def make_enhc (iter_index, 
    ...
    conf_list = glob.glob(mol_path + "conf*gro")  # origin mol file
    conf_list.sort()
    ...
    for walker_idx in range (numb_walkers) :
        ...
        # if int(iter_index) == 0 or int(walker_idx) < 6:
        ...
        if (iter_index > 0) :
            ...
            modified_conf_list = glob.glob("score/" + make_iter_name(iter_index-1) + "/selected*/conf_init.gro")
            modified_conf_list.sort()
            if os.path.isfile (prev_enhc_path + "confout.gro"):
                ...
                if walker_idx < 6:
                    ...
                    os.symlink (prev_enhc_path + "confout.gro", walker_path + "conf.gro")
                    if int(walker_idx) == 6:
                        modified_conf_file = modified_conf_list[0]
                        shutil.copy(modified_conf_file, walker_path + "conf_init.gro")
                    elif int(walker_idx) == 7:
                        modified_conf_file = modified_conf_list[1]
                        shutil.copy(modified_conf_file, walker_path + "conf_init.gro")
                    log_task ("use new cluster conf of iter " + make_iter_name(iter_index-1) + " walker " + make_walker_name(walker_idx) )
            else :
...
###########################################
        ...
        distance_list = get_distance(walker_path + "conf_init.gro")
...
def run_enhc (iter_index,
    ...
    gmx_prep_submission = Submission(work_base=work_path, resources=resources, machine=slurm, task_list=gmx_prep_task)
    ...
    gmx_run_submission = Submission(work_base=work_path, resources=resources, machine=slurm, task_list=gmx_run_task)
...
def clean_enhc(iter_index):
    ...
    cleaned_files = ['state*.cpt', 'traj.trr', 'topol.tpr', 'ener.edr', 'mdout.mdp']
    cwd = os.getcwd()
    for ii in all_task :
        if os.path.isdir(ii):
            os.chdir(ii)
